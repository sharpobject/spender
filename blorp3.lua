...
for batch_size=10,10 do
  ...
  --for batch_size = 1,1 do
    local target = {torch.Tensor(batch_size,220), torch.Tensor(batch_size,1)}
    ...
    for i=1,nr do
      ...
      print(kld.output * 220, mse.output, torch.exp(n.output[1][1][1]), n.output[2][1][1])
  ...
  for i=1,batch_size do
    local input = torch.Tensor(i,313)
    for j=1,i do
      game:dump_to_tensor(input[j])
    end
    local target = {torch.Tensor(i,220), torch.Tensor(i,1)}
    for j=1,i do
      target[1][j][1] = 1
      target[2][j][1] = 1
      for k=2,220 do
    ...
    --print(input)
    c:forward(n:forward(input), target)
    print(i, kld.output*220, mse.output)
    print("hi"..i,torch.exp(n.output[1][1][1]), n.output[2][1][1])
  end
  --[[input = GameState():as_tensor():view(1,313)
